...
\newline Find the cut/critical points(quantiles)  c$_{\alpha \\/ 2}$ such that:
\[  P \{ -c_{\alpha \\/ 2} \le Y \le c_{\alpha \\/ 2}  \}   = 1 - \alpha  \] 
c$_{\alpha \\/ 2}$ is the upper ($\alpha$ / 2)100th percentile.
\newline
\newline \textbf{Quantiles}- give you the area to the right of that point. They are the subscript of cut/critical points.\newline
...
\newline
\newline Both variance and standard deviation are non-negative numbers. \textul{The Chi-Square distribution allows us to construct confidence intervals for the variance and the standard deviation (when the original population of data is normally distributed).}
